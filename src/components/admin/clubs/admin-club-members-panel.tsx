...
import type { AdminClubMemberItem } from "@/lib/admin/clubs/clubs-admin-data"

interface AdminClubMembersPanelProps {
  clubId: string
...
export function AdminClubMembersPanel({
  clubId,
  members,
}: AdminClubMembersPanelProps) {
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      const result = await addAdminClubMember({ clubId, identifier, role: newRole })
  ...
  function handleRoleChange(userId: string, role: string) {
    ...
    startTransition(async () => {
      const result = await updateAdminClubMemberRole({ clubId, userId, role })
      setActiveUserId(null)
      if (!result.success) {
  ...
  function handleRemove(userId: string) {
    setActiveUserId(userId)
    startTransition(async () => {
      const result = await removeAdminClubMember({ clubId, userId })
      setActiveUserId(null)
      if (!result.success) {
  ...
  return (
    <Card>
      <CardContent className="flex flex-col gap-4 p-6">
        <div>
          <h2 className="text-lg font-semibold">Thành viên câu lạc bộ</h2>
          <p className="text-sm text-muted-foreground">
            Thêm thành viên bằng email hoặc mã sinh viên và phân quyền trong CLB.
          </p>
        </div>
          ...
          <table className="min-w-full border-separate border-spacing-0">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Vai trò CLB</th>
              ...
              {members.length === 0 && (
                <tr>
                  <td className="p-6 text-center text-sm text-muted-foreground" colSpan={4}>
                    Chưa có thành viên trong câu lạc bộ.
                  </td>
                </tr>
              )}
            </tbody>
